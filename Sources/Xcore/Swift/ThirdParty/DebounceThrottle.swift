...
extension TimeInterval {
    ...
    ///
    /// - Parameter since: The duration of time that needs to have passed for this
...
///
/// - Parameters:
///   - delay: A `DispatchTimeInterval` to wait before executing the wrapped
///     function after last invocation.
///   - queue: The queue to perform the action on.
///   - action: A function to debounce. Can't accept any arguments.
/// - Returns: A new function that will only call `action` if `delay` time
...
/// in every `delay` seconds.
///
/// - Parameters:
///   - delay: A `TimeInterval` specifying the number of seconds that needs to
///     pass between each execution of `action`.
///   - queue: The queue to perform the action on.
///   - action: A function to throttle.
/// - Returns: A new function that will only call `action` once every `delay`
///   seconds, regardless of how often it is called.
