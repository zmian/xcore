...
/// in the background for longer than the specified `sessionExpiration`, the
...
/// specified `sessionExpiration`, it is considered a new session, and all
/// cached values are cleared.
///
/// **Usage**
///
/// ```swift
/// let analyticsSessionCache = SessionAwareCache<String>(sessionExpiration: .seconds(30))
...
public final class SessionAwareCache<Value: Sendable & Hashable>: @unchecked Sendable {
    ...
    ///
    /// - Parameter duration: The duration after which the session expires if the
    ///   app remains in the background.
    ...
    ///
    /// When the app enters the background, the `lastActiveUptime` is recorded.
    /// When the app returns to the foreground, the session expiration is checked.
    /// If the session has expired, the cached values are cleared.
